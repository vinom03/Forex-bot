...
# ============================================================
# سجل موحد (log) يُحفظ بملف بالمستودع نفسه، عشان تقدر تفتح ملف
# وحد وتشوف تاريخ كل تشغيلة صارت (نجحت أو فيها خطأ) بدون ما تدخل
# كل تشغيلة لحالها بتبويب Actions.
# ============================================================
...
def get_channel_posts():
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_error = e
            log(f"محاولة {attempt}/{max_retries} فشلت (جلب المنشورات): {e}")
...
def download_image(url):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_error = e
            log(f"محاولة {attempt}/{max_retries} فشلت (تحميل صورة): {e}")
            if attempt < max_retries:
                time.sleep(3)
    raise last_error
...
def send_text(text):
    ...
    log(f"إرسال نص: {r.status_code} {r.text[:200]}")
...
def send_single_photo(photo_url, caption=""):
    ...
    log(f"إرسال صورة: {r.status_code} {r.text[:200]}")
    return r.ok and r.json().get("ok", False)
...
def send_media_group(photo_urls, caption=""):
    ...
    log(f"إرسال ألبوم: {r.status_code} {r.text[:200]}")
    return r.ok and r.json().get("ok", False)
...
def run_once():
    try:
        ...
        if not posts:
            log("ما فيه منشورات بالصفحة حالياً.")
            return
        ...
        if last_seen_id is None:
            ...
            log(f"بدأنا المراقبة من المنشور رقم: {latest_id}")
            return
        ...
        for post in posts:
            if post["id"] > last_seen_id:
                try:
                    ...
                    log(f"تم نسخ منشور جديد بنجاح ✅ {post['id']}")
