...
def clean_text(text_html):
    ...
    signature_line = re.compile(r'@GOLD_PRO804', re.IGNORECASE)  # 🔧 توقيع القناة المصدر
    ...
    if found_source_signature:
        log("🔏 المصدر سوى توقيعه بهالمنشور - راح يُحذف ويُضاف توقيعك بدله بعد الترجمة")
    else:
...
def add_own_signature(text, should_add_signature):
...
def translate_to_arabic(text, post_id=None):
    ...
    for attempt in range(1, TRANSLATE_MAX_RETRIES + 1):
        try:
            ...
            if translated:
                log(f"🌐 تم ترجمة المنشور {post_id} من الإنجليزي للعربي")
                return translated
            last_error = Exception("رجعت الترجمة فارغة")
        except Exception as e:
            last_error = e
            log(f"⚠️ محاولة {attempt}/{TRANSLATE_MAX_RETRIES} فشلت (ترجمة المنشور {post_id}): {e}")
            if attempt < TRANSLATE_MAX_RETRIES:
                time.sleep(TRANSLATE_RETRY_DELAY)

    log(f"⚠️ فشلت ترجمة المنشور {post_id} - راح يترسل بالإنجليزي كما هو ({last_error})")
    return text
    ...
    try:
        ...
        for post in new_posts:
            try:
                ...
                clean, should_add_signature = clean_text(post["text_html"])
                log(f"✏️ تم تنظيف نص المنشور {post['id']} (حذف توقيع المصدر إن وجد)")

                # ترجمة النص من الإنجليزي للعربي (القناة المصدر إنجليزية بالكامل)
                # مهم: هذا يصير قبل إضافة توقيعك، عشان توقيعك (العربي أصلاً)
                # ما يترسل لخدمة الترجمة ويتكسر رابطه أو تنسيقه
                clean = translate_to_arabic(clean, post_id=post["id"])

                # إضافة توقيعك الخاص بعد الترجمة مباشرة (لو المصدر سوى توقيعه)
                clean = add_own_signature(clean, should_add_signature)
